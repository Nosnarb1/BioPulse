...
// Intro curtain (neural mesh + DNA helix)
function initIntroCurtain() {
  const intro = document.querySelector(".bp-intro");
  if (!intro) return;

  // Lock scroll while intro runs
  document.body.classList.add("bp-intro-active");

  const finish = () => {
    intro.classList.add("bp-intro-hidden");
    document.body.classList.remove("bp-intro-active");
  ...
  // Safety timeout to end intro; matches curtain animation duration
